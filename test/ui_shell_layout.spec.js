...
describe("enterprise app shell", () => {
	...
	it("keeps memory content and onboarding aligned to the same visual system", () => {
		...
		expect(css).toMatch(/\.step-num \{[\s\S]{0,220}background: var\(--accent\); color: var\(--on-accent\);/);
		expect(css).toMatch(/\.step-row:not\(:last-child\) \.step-left::before \{[\s\S]{0,220}var\(--accent\)/);
	});
