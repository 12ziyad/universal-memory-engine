...
function result(year, monthIndex, day, precision, phrase) {
	...
	return {
		...
		epoch_ms: Date.UTC(year, monthIndex, day, 12, 0, 0),
	};
}
...
export function resolveTemporalPhrase(phrase, anchor) {
	...
	if (isoMatch) {
		...
		if (m >= 0 && m <= 11 && d >= 1 && d <= 31) return result(y, m, d, "day", phrase);
	}
	...
	if (dmy) {
		...
		if (m !== null) {
			...
			if (d >= 1 && d <= 31) return result(dmy[3] ? Number(dmy[3]) : year, m, d, "day", phrase);
		}
	}
	...
	if (mdy) {
		...
		if (m !== null) {
			...
			if (d >= 1 && d <= 31) return result(mdy[3] ? Number(mdy[3]) : year, m, d, "day", phrase);
		}
	}
	...
	if (ago) {
		...
		if (n !== null) {
			...
			if (ago[2] === "month") return result(year, monthIndex - n, day, "month", phrase);
			return result(year - n, monthIndex, day, "year", phrase);
		}
	}
	...
	if (edge) {
		...
		if (edge[3] === "month") {
			...
			return edge[1] === "end"
				? result(year, target + 1, 0, "month", phrase)
				: result(year, target, 1, "month", phrase);
		}
		return edge[1] === "end"
	...
	if (bareMonth) {
		const m = monthIndexOf(bareMonth[1]);
		if (m !== null) {
			// The most recent occurrence that is not in the future: "since March"
			// said in September means this year's March, said in February means
			// last year's.
			const y = m <= monthIndex ? year : year - 1;
			return result(y, m, 1, "month", phrase);
		}
	}
	// A bare year: "in 2019".
