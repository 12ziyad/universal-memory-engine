...
## E6 governed atomic projection (quality proven; MODIFY pending E6M)

```
completed E4/E5 capture-run ids
  -> tenant/project-bound candidate + exact source-episode join
  -> deterministic node + assertion proposal
  -> established rules/entity/conflict gates
  -> same-batch legacy/atomic coalescing
  -> one D1 transaction:
       semantic graph write/touch
       + candidate terminal state
       + semantic_atom_projections provenance ledger
       + extraction commit marker
```

The nested projection flag requires both parent V3 and atomic capture. It is off
by default and observable only as mode/count. Candidate ids and exact evidence
travel out-of-band from model output. A missing episode, mismatched scope,
missing semantic object, conflicting prior decision, or erasure fence aborts
the entire graph transaction.

Projected facts preserve `semantic_attribute`, `semantic_cardinality`, temporal
validity, and precision. A single-valued update retires only the same attribute;
multi-valued facts coexist. Relationships remain assertions in E6 so existing
graph strengths are preserved and any new relation lane can be attributed in
E7. Candidate-direct retrieval and episode fallback remain absent.

Migration 0037 is additive: one content-free projection ledger, nullable typed
fields on slices/events, and indexes. Projection rows are hard-deleted before
their candidates/source episodes and participate in account-erasure convergence.
The three-seed non-LoCoMo ablation established a large general-quality gain:
capture recall 90.30%, evidence availability 79.37% and judge accuracy 84.92%,
all materially above paired controls with 100% projection/provenance
accounting. It is not yet adopted because same-source legacy and atomic
paraphrases raised semantic redundancy by 7.76pp.

E6M adds no new memory lane or inference. It conservatively maps an atomic
proposal to an already planned legacy assertion only when scope, resolved node,
assertion class, exact source evidence and high lexical containment all agree.
The coalesced assertion inherits atomic projection provenance and temporal/type
metadata. Different facts sharing a message never coalesce. Physical/source-
identity duplication, intentional representation overlap and final rendered-
context duplication are measured separately; this prevents an ontology view
from being mistaken for uncontrolled duplicate storage.

## V3-D13 bounded E7 corpus construction

The E7 output limit is now also a database-to-Worker resource bound. Under the
nested E7 flag, exact/FTS/assertion/temporal/vector/graph lanes apply tenant and
project predicates before SQL limits, candidate identities are fused fairly,
and only a fixed evidence closure is hydrated. Lane failure can degrade only to
another bounded lane or bounded recent candidates; it cannot reopen the legacy
full-corpus scan. Accounts outside E7 retain the unchanged legacy path.
...
E9A does not search raw conversations. After E7 has selected and bounded the
semantic assertions, it follows only durable exact projection provenance:

```
selected assertion (max 12)
  -> semantic_atom_projections
  -> semantic_atom_candidates
  -> source_episodes (max 8 unique)
  -> canonical re-scrub
  -> max 700 Unicode code points per source
  -> shared final 24,000-character context cap
```

Tenant, project, source packet, episode, message, memory-user, owner-user and
external-user identities are constrained in SQL before text can be returned.
Missing or inconsistent provenance fails open only to the unchanged semantic
context and emits content-free telemetry. The feature is nested below parent
V3 and E7 and is account-scoped; health exposes only mode/count.

The paired frozen holdout improved judge 95.24% -> 97.62% and token-F1 65.38%
-> 72.83% without changing evidence availability. Conditional accuracy reached
100%; no lookup failed; recall p95 stayed at the registered +25% boundary.
This earns exact source expansion, not episode search. Episode vectors remain
absent.
