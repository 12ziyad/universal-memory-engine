...
| **V3-D14** | **MEDIUM** | **PRODUCT OBSERVABILITY / RECOVERY ACCOUNTING DEFECT** | A crash after the atomic graph/projection commit but before receipt publication left one historical Stage E receipt undercounting projection telemetry by 20 even though D1 had exact 392 candidates and 392 projections. Recovery rebuilt capture counters but not projection counters. | **IMPLEMENTED + FULLY GATED LOCALLY; DEPLOYMENT PENDING.** Recovery now derives content-free projection counters from the tenant/run-bound append-only ledger and repairs the returned response without mutating immutable historical receipts. Failing-first plus all complete gates pass. Durable state was never missing; the final audit records the historical receipt gap separately. Evidence: `final/locomo/V3-D14-PROJECTION-RECOVERY-ACCOUNTING.md`. |
