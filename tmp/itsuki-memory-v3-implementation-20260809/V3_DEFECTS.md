...
## Final Stage E boundedness and accounting findings (2026-08-12)

| id | sev | classification | finding | status |
|---|---|---|---|---|
| **V3-D15** | **HIGH** | **PRODUCT BOUNDEDNESS / INFERENCE-COST DEFECT** | Two project-owned `meaningful_no_write` rescue buffers could ping-pong forever after both memory jobs were already terminal. Held-to-queue transitions reset ordinary attempts, so the per-entry poison ceiling did not bound the rescue lineage. Reference-blind detection on erased synthetic sample `conv-43` reached 1,740 extraction runs and added 9,671 Qwen neurons before containment; no tenant leak, answer, reference, judge or score occurred. | **CLOSED.** Repair commit/origin `fb1c7bc322a5fc26c04765b22d6fdb344317b2a3` is deployed as Worker `24a9f822-e325-4ee6-91c1-1794e64ae857`, deployment `2b0c9086-36bf-4f30-a367-94b71cbc443c`. The valid production reattack proved exactly three bounded terminal runs, project reconsideration, tenant separation, zero semantic writes and complete cleanup. Only erased `conv-43` was rebuilt: 29 sessions, 680 episodes and 35 packets. The full reference-blind audit then passed 5,882/5,882 episodes and 5,572/5,572 candidate/projection conservation. No migration. Full evidence: `final/locomo/V3-D15-NO-WRITE-RESCUE-STORM.md`, `final/locomo/evidence/V3-D15-PRODUCTION-REATTACK-ATTEMPT-002.json`, `final/locomo/evidence/V3-D15-CONV-43-REBUILD.json`, and `final/locomo/evidence/state-audit.json`. |
| **V3-D14** | **MEDIUM** | **PRODUCT OBSERVABILITY / RECOVERY ACCOUNTING DEFECT** | A crash after the atomic graph/projection commit but before receipt publication left one historical Stage E receipt undercounting projection telemetry by 20 even though D1 had exact 392 candidates and 392 projections. Recovery rebuilt capture counters but not projection counters. | **CLOSED.** Recovery derives content-free projection counters from the tenant/run-bound append-only ledger and repairs the returned response without mutating immutable historical receipts. It shipped with V3-D15 in commit/origin `fb1c7bc322a5fc26c04765b22d6fdb344317b2a3`, Worker `24a9f822-e325-4ee6-91c1-1794e64ae857`; the final reference-blind audit reconciled exact 5,572/5,572 candidate/projection state. Durable state was never missing. Evidence: `final/locomo/V3-D14-PROJECTION-RECOVERY-ACCOUNTING.md`. |

## Final Stage E harness findings (2026-08-12)

| id | sev | classification | finding | status |
|---|---|---|---|---|
| **V3-H24** | **HIGH** | **HARNESS DEFECT / JUDGE TRANSPORT INVALIDATION** | During the owner-authorized Stage E judge tail, the local Wrangler evaluator exited after repeated internal loopback/Workers AI errors. The frozen judge client exhausted five retries and represented 40 transport failures as WRONG even though no model verdict existed. The preserved 960-row prefix had zero errors; the defect was detected before a complete score artifact existed. A later single request also remained open for more than 90 seconds after evaluator internal errors, without appending a row. | **CLOSED.** The full 1,260-row interrupted ledger is retained privately and marked invalid. Only the 40 `fetch failed` identities were quarantined; every actual model verdict remained, including the sealed original 960. The scorer now aborts before appending a transport-failed verdict, enforces a 90-second no-verdict watchdog, and runs at concurrency one; prompt, model, temperature, token budget, answers, references and verdict semantics were unchanged. The exact frozen ledger subsequently completed 1,540/1,540 unique verdicts with zero judge errors counted wrong. Evidence: `final/locomo/evidence/judge-tail-invalid-transport-attempt-001.json` and `final/locomo/results/stage-e-terminal-summary.json`. |
| **V3-H17** | **MEDIUM** | **HARNESS DEFECT / REPAIR SHAPE** | The exact D04 repair verifier assumed exactly two atomic runs, but deterministic pre-splitting can legitimately retain a terminal empty tail between completed chunks. | **CLOSED BEFORE ANSWER/SCORE.** Repair proof now accepts bounded terminal empty chunks while requiring exact candidate/projection conservation; the authoritative D04 production artifact records 11 + empty + 9 and 8 + 2. |
| **V3-H18** | **MEDIUM** | **HARNESS DEFECT / RETRY ACCOUNTING** | The final state audit rejected honest interrupted predecessor history, double-counted old receipts, and assumed exactly one extraction/pass-2 shape per packet. | **CLOSED BEFORE ANSWER/SCORE.** Retry-aware reconciliation uses successful extraction-run-linked canonical receipts, permits only generation-authorized `inference_outcome_unknown` predecessors, and separately proves terminal jobs, atomic runs, candidates and projections. Pure helper tests pass 4/4. |
| **V3-H19** | **MEDIUM** | **HARNESS DEFECT / PROVENANCE SHAPE** | The audit required every object source snippet to equal a current atomic quote, rejecting legitimate legacy-object reinforcement whose immutable provenance is the scrubbed source episode or normalized 240-character snippet. | **CLOSED BEFORE ANSWER/SCORE.** The verifier accepts only exact quote, normalized/truncated quote, or exact governed episode provenance; scope, source packet, episode and object bindings remain mandatory. |
| **V3-H20** | **MEDIUM** | **HARNESS DEFECT / RETIRED LINEAGE ACCOUNTING** | The first containment rebuild checker counted confirmed-delete extraction-run tombstones as active work. Cleanup intentionally retains those rows with `status='deleted'` for audit, so the checker falsely reported the previously erased rescue storm as a new runaway. | **CLOSED BEFORE ANSWER/SCORE.** Corrected accounting separates 1,756 retired audit rows from active lineage, proves zero active runs/jobs and zero live semantic/source content before rebuild, and retains strict per-active-packet bounds. The first one-session rebuild attempt was safely erased; the validated 29-session rebuild and complete state audit passed. |
| **V3-H21** | **MEDIUM** | **HARNESS DEFECT / OFFICIAL SCORER RUNTIME RECOVERY** | After the 1,540-answer product sealed, the score phase referenced a removed campaign-local Python virtual environment. It durably wrote the exact 1,540-row official-scorer input, then failed with `ENOENT` before official output or judge row 1. The original accepted LoCoMo benchmark virtual environment remained intact. | **CLOSED BEFORE JUDGE INFERENCE.** Interpreter discovery now falls back to that preserved Python 3.12.10 environment, whose required `nltk`/`numpy` imports pass. Deterministic recovery accepts only an exact existing scorer input with absent output, revalidates it against the sealed product/reference join, and reruns only the inference-free official scorer. Output-without-input remains fail-closed. No answer, judge protocol, reference, or metric changed. |
| **V3-H22** | **MEDIUM** | **HARNESS DEFECT / LARGE-COHORT DELETE TIMEOUT** | The generic campaign erasure helper used a 90-second client timeout. A complete Stage E tenant could exceed it even though confirmed deletion was idempotent and continued safely server-side; the first cleanup attempt stopped after three tenants and left seven for explicit verification. | **CLOSED BEFORE FINAL CLEANUP.** Stage E uses the same confirmed, product-fenced deletion contract with one 300-second client window, verifies residue and nonterminal jobs after every tenant, and performs no schema/data repair. The exact replacement cleanup erased all ten tenants and passed every zero-state gate. |
| **V3-H23** | **MEDIUM** | **HARNESS DEFECT / CLEANUP PROOF ORDERING** | Post-erasure zero-recall verification correctly creates one governed query audit packet per tenant. The first terminal audit checked packet-content zero immediately afterward and therefore found cleanup's own ten query packets, not retained user memory. | **CLOSED BEFORE FINAL VERDICT.** Recall and export proofs still execute and must be empty; the proof packets are then erased through the same product contract before a second complete store/FTS/packet audit. Final result: 2,096/2,096 minimized packet fences and zero content rows. No product privacy defect or benchmark residue remained. |
...
| id | sev | classification | finding | status |
|---|---|---|---|---|
| **V3-D04** | **HIGH** | **PRODUCT DURABILITY / REPLAY DEFECT** | In each of two accepted multi-chunk packets, one atomic chunk committed and a later chunk was interrupted. Failed settlement polluted the DO `seen` set; exact replay did not restore `payload.remaining`, reused the old handoff identity, and could falsely settle `enriched`; stale atomic runs were unreclaimable and unfenced. A post-CAS concurrent follower and stale older-generation local ownership had the same zero-write/stale-settlement risk. Allowed accepted source survived as episodes, but the interrupted semantic suffix could remain permanently uncaptured. | **CLOSED.** Nine failing-first boundaries and complete gates passed at commit `5b7ffec8fed8b2481340393b7615069ac9859306`; production Worker `29bed9a0-78cb-44ee-a6b4-59cd4f8d7f3c`, deployment `6d38a27a-972e-47f5-8bd6-9d7a6ac2df1d`. Reference-blind exact production proof completed both packets with generation 1, attempts 1, zero nonterminal work, and exact 20/20 plus 10/10 candidate/projection conservation. Full lifecycle evidence: `final/locomo/V3-D04-INTERRUPTED-CAPTURE-REPAIR.md` and `final/locomo/evidence/V3-D04-PRODUCTION-REATTACK.json`. |
...
| id | sev | title | status |
|---|---|---|---|
| BF-1 | HIGH | `/v1/ingest` has no authoritative source/session timestamp field. `ts` is accepted by `normalizeMessageBatch` but is not on the wire contract and defaults to `Date.now()` (`src/pipeline/source.js:201,270`), so it records *ingested_at*, not *source_time*. Relative phrases ("yesterday", "last week") have no anchor. Leading explanation for temporal at 6.23%. | **CLOSED / HISTORICAL V1 FINDING.** The backward-compatible source-time contract, persistence and deterministic temporal normalization shipped and passed SDK/REST/replay/timezone/DST gates; final temporal token-F1 is 33.63%. |
| BF-2 | HIGH | `/v1/recall` allowlists `limit` (`src/lib/params.js:47`) but the handler never forwards it (`src/index.js:1631-1640`), and `runRecallCommand` never forwards it to `recall` (`src/pipeline/commands.js:527-531`). A documented parameter silently does nothing. | **CLOSED / HISTORICAL V1 FINDING.** Bounded candidate and final-evidence limits are enforced; E1 and final depth-200 paths ran under hard 200-item / 24,000-character bounds, and V3-D13 closed pre-fusion unbounded loading. |
| BF-3 | HIGH | Extraction coverage is the binding constraint: 64.6% of judge-misses are Stage A (never stored). ~1 memory per 7.5 turns; whole-conversation graphs average 78 nodes. | **ADDRESSED / NOT AN OPEN PRODUCT DEFECT.** E4 source-grounded atomic capture plus acceptance-atomic episodes raised final source storage to 94.61% and semantic candidate availability to 85.32%. Residual 226 semantic misses remain a measured quality weakness, not an unresolved safety/durability defect. |
