...
|---|---|---|---|---|
| **V3-D15** | **HIGH** | **PRODUCT BOUNDEDNESS / INFERENCE-COST DEFECT** | Two project-owned `meaningful_no_write` rescue buffers could ping-pong forever after both memory jobs were already terminal. Held-to-queue transitions reset ordinary attempts, so the per-entry poison ceiling did not bound the rescue lineage. Reference-blind detection on erased synthetic sample `conv-43` reached 1,740 extraction runs and added 9,671 Qwen neurons before containment; no tenant leak, answer, reference, judge or score occurred. | **CONTAINED; IMPLEMENTED + FULLY GATED LOCALLY; DEPLOYMENT / PRODUCTION REATTACK PENDING.** The exact affected tenant was erased through the product contract to zero. A persisted rescue-generation counter now allows one deliberate reconsideration and then terminates the lineage; pre-fix flag-only state upgrades fail closed to the consumed generation. Failing-first 3/4 becomes 4/4 and exactly three runs; adjacent 50/50; Worker 1,324/1,324; unit 539/539 + one skip; audit/diff/dry deploy pass. No migration. Full evidence: `final/locomo/V3-D15-NO-WRITE-RESCUE-STORM.md`. |
