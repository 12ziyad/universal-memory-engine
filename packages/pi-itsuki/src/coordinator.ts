 ...
 *
 * Nothing in this file knows what Pi is. It answers two questions — "what
 * should I inject before this turn?" and "what should I persist after it?" —
 * and owns the failure policy for both:
 *
 ...
 *   if it is ever genuinely lost, counted and reported. Nothing is ever
 *   reported as saved without a receipt.
 */
...
export class Coordinator {
	...
	/** Fingerprints of context injected this session, for echo suppression. */
	private injected = new Set<string>();
	private echoKey: string | null = null;
	...
	setEchoKey(key: string | null): void {
		this.echoKey = key;
	}
	...
	async recall(query: string, signal?: AbortSignal): Promise<RecallOutcome> {
		if (!this.recallConfig.enabled) return { block: null, status: "disabled", count: 0 };
		const trimmed = String(query ?? "").trim();
		if (!trimmed) return { block: null, status: "empty", count: 0 };
		if (this.breakerOpen()) {
			this.log(`itsuki recall skip code=breaker_open`);
			return { block: null, status: "breaker_open", count: 0, code: this.breakerReason ?? "unknown" };
		}
		...
		try {
			...
			if (block && this.echoKey) {
				for (const fingerprint of echoFingerprints(String(context ?? ""), this.echoKey)) {
					this.injected.add(fingerprint);
				}
			}
			this.log(`itsuki recall ok count=${count} ms=${this.now() - started}`);
			return block
				? { block, status: "injected", count }
				: { block: null, status: "empty", count };
		} catch (error) {
	...
	/** Scrub + echo-suppress one message's text. */
	transformCaptured = (text: string, _role: "user" | "assistant"): string => {
		const scrubbed = scrubText(text).text;
		return this.echoKey ? suppressEchoLines(scrubbed, this.injected, this.echoKey) : scrubbed;
	};

	/**
	 * Take durable ownership of a span. No network, so this cannot fail
	 * halfway across a wire.
	 *
	 * Staging is deliberately separate from delivery, and the caller must
	 * advance its watermark between the two. If the process dies after staging
	 * but before delivery, the next run drains the identical envelope under the
	 * identical content-derived key. If it died BEFORE the watermark advanced,
	 * the span is simply recollected — and hashes to the same key again. Either
	 * way the server sees one write.
	 */
	async stage(messages: CaptureMessage[]): Promise<StageOutcome> {
		...
		for (const batch of batches) {
			const idempotencyKey = captureIdentity(this.scope, batch);
			...
			const envelope: SpoolEnvelope = {
				...
				scope: {
					...
					conversationId: this.scope.conversationId ?? null,
