...
beforeEach(async () => {
	root = await mkdtemp(join(tmpdir(), "pi-itsuki-spool-"));
});
...
describe("durability", () => {
	it("round-trips an envelope through the filesystem", async () => {
		...
		expect(await spool.enqueue(envelope("pi:v1:a"))).toBe(true);
		const listed = await spool.list();
		expect(listed.length).toBe(1);
		expect(listed[0]!.envelope.idempotencyKey).toBe("pi:v1:a");
	...
	it("leaves no partial file behind: only completed envelopes are visible", async () => {
		const spool = new Spool(root);
		await spool.enqueue(envelope("pi:v1:a"));
	...
	it("survives a process restart — a new instance sees the same queue", async () => {
		await new Spool(root).enqueue(envelope("pi:v1:a"));
...
describe("idempotent ownership", () => {
	it("enqueues the same span only once", async () => {
		const spool = new Spool(root);
		expect(await spool.enqueue(envelope("pi:v1:same"))).toBe(true);
		expect(await spool.enqueue(envelope("pi:v1:same"))).toBe(false);
	...
	it("keeps distinct spans distinct", async () => {
		const spool = new Spool(root);
		await spool.enqueue(envelope("pi:v1:a"));
		await spool.enqueue(envelope("pi:v1:b"));
...
describe("bounds and honesty", () => {
	it("drops the oldest when full, and counts the loss", async () => {
		...
		for (let i = 0; i < SPOOL_LIMITS.maxEntries; i += 1) {
			await spool.enqueue({ ...envelope(`pi:v1:${i}`), createdAt: new Date(1_000 + i).toISOString() });
		}
		expect((await spool.stats()).depth).toBe(SPOOL_LIMITS.maxEntries);
		expect((await spool.stats()).dropped).toBe(0);

		await spool.enqueue({ ...envelope("pi:v1:new"), createdAt: new Date(9_999_999).toISOString() });
		...
		expect(keys).toContain("pi:v1:new");
		expect(keys).not.toContain("pi:v1:0");
	});

	it("refuses an oversized envelope and counts that too", async () => {
		const spool = new Spool(root);
		const huge = envelope("pi:v1:huge", "x".repeat(SPOOL_LIMITS.maxEnvelopeBytes + 10));
	...
	it("ignores an envelope written by a future schema rather than mis-sending it", async () => {
		...
		await writeFile(
			...
			JSON.stringify({ ...envelope("pi:v1:future"), schema: "itsuki.pi-spool/v2" }),
			"utf8",
		);
		expect((await spool.list()).length).toBe(0);
	});
...
describe("delivery lifecycle", () => {
	...
	it("drains oldest-first", async () => {
		const spool = new Spool(root);
		await spool.enqueue({ ...envelope("pi:v1:second"), createdAt: new Date(2_000).toISOString() });
		await spool.enqueue({ ...envelope("pi:v1:first"), createdAt: new Date(1_000).toISOString() });
		const order = (await spool.list()).map((e) => e.envelope.idempotencyKey);
		expect(order).toEqual(["pi:v1:first", "pi:v1:second"]);
	});
});
